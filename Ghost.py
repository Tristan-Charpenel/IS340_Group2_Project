...
class Ghost:
    ...
    def update(self):
        ...
        if self.target == [-1, -1] or (self.row == self.target[0] and self.col == self.target[1]) or init.gameBoard[int(self.row)][int(self.col)] == 4 or self.dead:
            self.setTarget()
        self.setDir()
        self.move()
        ...
        if self.dead and init.gameBoard[self.row][self.col] == 4:
            self.deathCount += 1
            self.attacked = False
    ...
    def draw(self): # Ghosts states: Alive, Attacked, Dead Attributes: Color, Direction, Location
        ghostImage = pygame.image.load(init.ElementPath + "tile152.png")
        ...
        if self.dead:
            ...
            ghostImage = pygame.image.load(init.ElementPath + "tile" + str(tileNum) + ".png")
        elif self.attacked:
            if self.attackedTimer - self.attackedCount < self.attackedTimer//3:
                if (self.attackedTimer - self.attackedCount) % 31 < 26:
                    ghostImage = pygame.image.load(init.ElementPath + "tile0" + str(70 + (currentDir - (((self.dir + 3) % 4) * 2))) + ".png")
                else:
                    ghostImage = pygame.image.load(init.ElementPath + "tile0" + str(72 + (currentDir - (((self.dir + 3) % 4) * 2))) + ".png")
            else:
                ghostImage = pygame.image.load(init.ElementPath + "tile0" + str(72 + (currentDir - (((self.dir + 3) % 4) * 2))) + ".png")
        else:
            if self.color == "blue":
                ...
                if tileNum < 100:
                    ghostImage = pygame.image.load(init.ElementPath + "tile0" + str(tileNum) + ".png")
                else:
                    ghostImage = pygame.image.load(init.ElementPath + "tile" + str(tileNum) + ".png")

        ghostImage = pygame.transform.scale(ghostImage, (int(init.square * init.spriteRatio), int(init.square * init.spriteRatio)))
        init.screen.blit(ghostImage, (self.col * init.square + init.spriteOffset, self.row * init.square + init.spriteOffset, init.square, init.square))

    def isValidTwo(self, cRow, cCol, dist, visited):
        if cRow < 3 or cRow >= len(init.gameBoard) - 5 or cCol < 0 or cCol >= len(init.gameBoard[0]) or init.gameBoard[cRow][cCol] == 3:
            return False
    ...
    def setTarget(self):
        if init.gameBoard[int(self.row)][int(self.col)] == 4 and not self.dead:
            self.target = [ghostGate[0][0] - 1, ghostGate[0][1]+1]
            return
        elif init.gameBoard[int(self.row)][int(self.col)] == 4 and self.dead:
        ...
        while True:
            ...
            if self.target[0] <= 15 and self.target[1] >= 13:
                ...
                quad = 3
            if not init.gameBoard[self.target[0]][self.target[1]] == 3 and not init.gameBoard[self.target[0]][self.target[1]] == 4:
                break
            elif quads[quad] == 0:
                break
    ...
    def move(self):
        ...
        self.col = self.col % len(init.gameBoard[0])
        if self.col < 0:
            self.col = len(init.gameBoard[0]) - 0.5
